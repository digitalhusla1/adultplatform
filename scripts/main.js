...
async function initSearchPage() {
    ...
    try {
        ...
        if (resultsInfo) {
            if (data.total_count === 0) {
                resultsInfo.textContent = 'No videos found.';
                document.getElementById('noResults').style.display = 'block';
                container.style.display = 'none';
            } else {
                resultsInfo.textContent = `Found ${data.total_count} results`;
        ...
        container.innerHTML = '<div class="no-results"><p>Error loading search results.</p></div>';
        console.error('Search page error:', error);
    }
}
...
async function initVideoPage() {
    ...
    try {
        ...
        // Render embed
        container.innerHTML = `<iframe src="${video.embed}" allowfullscreen></iframe>`;
        ...
        document.getElementById('videoViews').innerHTML = `Views: <strong>${formatViews(video.views)}</strong>`;
        document.getElementById('videoRating').innerHTML = `Rating: <strong>${video.rate}/5</strong>`;
        document.getElementById('videoDuration').innerHTML = `Duration: <strong>${video.length_min}</strong>`;

        // Add tags
        if (video.keywords) {
            const tags = video.keywords.split(',').map(tag => tag.trim());
            const tagsContainer = document.getElementById('videoTags');
            tagsContainer.innerHTML = tags
                .slice(0, 15) // Limit to 15 tags
                .map(tag => `<span class="tag" onclick="searchTag('${escapeHtml(tag)}')">${escapeHtml(tag)}</span>`)
                .join('');
        }

        // Setup share and report buttons
        ...
        document.getElementById('reportBtn').addEventListener('click', () => {
            window.location.href = `contact.html?subject=report&video=${videoId}`;
        });
